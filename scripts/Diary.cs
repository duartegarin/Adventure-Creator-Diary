...
public class Diary : MonoBehaviour {
	...
	GameObject pageLeft;
	//Page Right Button
	GameObject pageRight;

    void Start()
    {
		//Get button references
		pageLeft = gameObject.transform.parent.Find ("PageLeft").gameObject;
		pageRight = gameObject.transform.parent.Find ("PageRight").gameObject;

        float scaleFactor = 1;
        if (canvas) scaleFactor = canvas.scaleFactor;
        float pageWidth = (DiaryPanel.rect.width* scaleFactor - 1) / 2;
        float pageHeight = DiaryPanel.rect.height* scaleFactor;
        Left.gameObject.SetActive(false);
        Right.gameObject.SetActive(false);
        UpdateSprites();
        Vector3 globalsb = DiaryPanel.transform.position + new Vector3(0, -pageHeight / 2);
        sb = transformPoint(globalsb);
        Vector3 globalebr = DiaryPanel.transform.position + new Vector3(pageWidth, -pageHeight / 2);
        ebr = transformPoint(globalebr);
        Vector3 globalebl = DiaryPanel.transform.position + new Vector3(-pageWidth, -pageHeight / 2);
        ebl = transformPoint(globalebl);
        Vector3 globalst = DiaryPanel.transform.position + new Vector3(0, pageHeight / 2);
    ...
    {
        Vector2 localPos = DiaryPanel.InverseTransformPoint(global);
        //RectTransformUtility.ScreenPointToLocalPointInRectangle(DiaryPanel, global, null, out localPos);
        return localPos;
    }
    void Update()
    {
    ...
    public void UpdateDiary()
    {
        ...
        if (mode == FlipMode.RightToLeft)
            UpdateDiaryRTLToPoint(f);
        else
            UpdateDiaryLTRToPoint(f);
    }
    public void UpdateDiaryLTRToPoint(Vector3 followLocation)
    {
        ...
        Right.transform.SetParent(DiaryPanel.transform, true);
        LeftNext.transform.SetParent(DiaryPanel.transform, true);
        ...
        ClippingPlane.transform.position = DiaryPanel.TransformPoint(t1);

        //page position and angle
        Left.transform.position =DiaryPanel.TransformPoint( c);
        ...
        NextPageClip.transform.position = DiaryPanel.TransformPoint(t1);
    ...
    public void UpdateDiaryRTLToPoint(Vector3 followLocation)
    {
        ...
        Left.transform.SetParent(DiaryPanel.transform, true);
        RightNext.transform.SetParent(DiaryPanel.transform, true);
        c = Calc_C_Position(followLocation);
        Vector3 t1;
        ...
        //page position and angle
        Right.transform.position = DiaryPanel.TransformPoint(c);
        float C_T1_dy = t1.y - c.y;
        float C_T1_dx = t1.x - c.x;
        float C_T1_Angle = Mathf.Atan2(C_T1_dy, C_T1_dx) * Mathf.Rad2Deg;
...
//                {
//                    UpdateSprites();
//                    RightNext.transform.SetParent(DiaryPanel.transform);
//                    Right.transform.SetParent(DiaryPanel.transform);
//
//                    Left.gameObject.SetActive(false);
//                    Right.gameObject.SetActive(false);
//                    pageDragging = false;
...
//                    UpdateSprites();
//
//                    LeftNext.transform.SetParent(DiaryPanel.transform);
//                    Left.transform.SetParent(DiaryPanel.transform);
//
//                    Left.gameObject.SetActive(false);
//                    Right.gameObject.SetActive(false);
//                    pageDragging = false;
...
//                ));
    ...
    public IEnumerator TweenTo(Vector3 to, float duration, System.Action onFinish)
    {
        ...
        {
            if(mode== FlipMode.RightToLeft)
            UpdateDiaryRTLToPoint( f + displacement);
            else
                UpdateDiaryLTRToPoint(f + displacement);
	...
	/**
	 * Method to add a new page to the Journal
	 * @param sprite - The new image page to add
	 * */
	public void addPage(Sprite sprite){
		Sprite[] temp_bookPages = new Sprite[TotalPageCount+1];
		bookPages.CopyTo (temp_bookPages,0);
		bookPages = temp_bookPages;
		bookPages[TotalPageCount-1] = sprite;
	...
	/**
	 * Method to update prev/next buttons if a new page has been added or journal has been flipped.
	 * */
	public void updateButtons(){
		if (currentPage > 1) {
			pageLeft.GetComponent<CanvasGroup> ().alpha = 1;
		}
		if (currentPage < TotalPageCount - 1) {
			pageRight.GetComponent<CanvasGroup> ().alpha = 1;
		}
		if (currentPage <= 1) {
			pageLeft.GetComponent<CanvasGroup> ().alpha = 0;
		}
		if (currentPage >= TotalPageCount - 1) {
			pageRight.GetComponent<CanvasGroup> ().alpha = 0;
		}
	}
}
